    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is None:
                ...
                # BEGIN Problem 9 
                if insect.can_contain(self.ant) and not self.ant.container:
#                    m = self.ant
#                    self.ant = insect
#                    self.ant.ant = m
                    self.ant, self.ant.ant = insect, self.ant
                    
                elif self.ant.can_contain(insect) and not insect.container:
                    self.ant.contain_ant(insect)
#                if debug:
#                    print('self.ant is', self.ant)
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
#        return self.container and self.ant == None and other.container is False
        return self.container and other.container!=True and self.ant==None
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while ant_place != hive :
            if ant_place.bees and place <= self.max_range and place >= self.min_range:
...
#        min_range = 5
#        while ant_place != hive:
#            if ant_place.bees and range >= min_range :
#                return random_or_none(ant_place.bees)
#            ant_place = ant_place.entrance
#            range += 1
    # END Problem 4
...
#        range = 0
#        max_range = 3
#        while ant_place != hive and range <= max_range:
#            if ant_place.bees :
#                return random_or_none(ant_place.bees)
#            ant_place = ant_place.entrance
#            range += 1
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn but it is useful because it has a large armor value"""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a subclass of ThrowerAnt that is more costly and watersafe, but otherwise identical to its base class. A ScubaThrower should not lose its armor when placed in Water."""
        ...
        if self.queen == True:
            ...
            while location.exit != None:
                if location.exit.ant != None:
                    if location.exit.ant.container:
                        if location.exit.ant not in self.ants_double:
                            location.exit.ant.damage += location.exit.ant.damage
                            self.ants_double.append(location.exit.ant)
                        if location.exit.ant.ant != None and location.exit.ant.ant not in self.ants_double and isinstance(location.exit.ant.ant, QueenAnt) == False :
                            location.exit.ant.ant.damage += location.exit.ant.ant.damage
                            self.ants_double.append(location.exit.ant.ant)
                    
                    else:
                        if location.exit.ant not in self.ants_double and isinstance(location.exit.ant, QueenAnt) == False:
                            location.exit.ant.damage += location.exit.ant.damage
                            self.ants_double.append(location.exit.ant)
